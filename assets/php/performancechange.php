...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (!empty($test1) && is_numeric($test1) && ($test1 < 0 || $test1 > 100)) {
        $errors['test1Input'] = "The value is only in the range 0 -> 100";
  }
  if (!empty($test2) && !is_numeric($test2)) {
        $errors['test2Input'] = "Please enter the number";
  }
  if (!empty($test2) && is_numeric($test2) && ($test2 < 0 || $test2 > 100)) {
        $errors['test2Input'] = "The value is only in the range 0 -> 100";
  }
  if (!empty($final) && !is_numeric($final)) {
        $errors['finalInput'] = "Please enter the number";
  }
  if (!empty($final) && is_numeric($final) && ($final < 0 || $final > 100)) {
        $errors['finalInput'] = "The value is only in the range 0 -> 100";
  }
  if (!empty($$attendance) && !is_numeric($attendance)) {
        $errors['attendanceInput'] = "Please enter the number";
  }
  if (!empty($attendance) && is_numeric($attendance) && ($attendance < 0 || $attendance > 100)) {
        $errors['attendanceInput'] = "The value is only in the range 0 -> 100";
  }

  if (!empty($homework) && !is_numeric($homework)) {
        $errors['attendanceInput'] = "Please enter the number";
  }
  if (!empty($$homework) && is_numeric($$homework) && ($homework < 0 || $homework > 100)) {
        $errors['attendanceInput'] = "The value is only in the range 0 -> 100";
  }
